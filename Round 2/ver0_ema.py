...
class Trader:
    ...
    def update_ema_prices(self, state: TradingState):
        ...
        for product in PRODUCTS:
            ...
            if self.ema_prices[product] is None:
                ...
                self.ema_prices[product] = self.ema_param[product] * mid_price + \
                    (1-self.ema_param[product]) * self.ema_prices[product]
    ...
    def orchids_strategy(self, state: TradingState):
        ...
        acceptable_bid_price = self.ema_prices[ORCHIDS] - 1
        acceptable_ask_price = self.ema_prices[ORCHIDS]

        if converse.bidPrice != None and position_orchids != 0:
            if int(converse.bidPrice) > acceptable_ask_price:
                print("SELL", str(bid_volume) + "x", converse.bidPrice, " ")
                orders.append(
                    Order(ORCHIDS, int(converse.bidPrice), max(-30, ask_volume)))
            ...
            if int(best_bid) > acceptable_ask_price:
                ...
                orders.append(Order(ORCHIDS, best_bid, max(
                    ask_volume, -best_bid_amount)))

        if converse.askPrice != None:
            if position_orchids < 0 and int(converse.askPrice) < acceptable_bid_price:
    ...
    def run(self, state: TradingState):
        ...
        # ORCHIDS STRATEGY
        try:
            result[ORCHIDS] = self.orchids_strategy(
                state)
