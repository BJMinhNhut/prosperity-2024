...
class Trader:
    ...
    def orchids_strategy(self, state: TradingState):
        ...
        acceptable_ask_price = self.ema_prices[ORCHIDS] + 1

        if state.timestamp >= 99000:
            if position_orchids < 0 and len(order_depth.sell_orders) != 0:
            ...
            if position_orchids > 0 and len(order_depth.buy_orders) != 0:
                best_bid, best_bid_amount = list(
                    order_depth.buy_orders.items())[0]
                if int(best_bid) > acceptable_ask_price:
                    print("SELL", str(best_bid_amount) + "x", best_bid)
                    orders.append(Order(ORCHIDS, best_bid, -position_orchids))
            return orders
        ...
        if position_orchids < -50 and converse.askPrice != None:
