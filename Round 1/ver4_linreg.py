...
class Trader:
    ...
    def update_ema_prices(self, state: TradingState):
        ...
        for product in PRODUCTS:
            ...
            if self.ema_prices[product] is None:
                ...
                delta = mid_price - self.ema_prices[product]
                if delta > 0:
    ...
    def starfruit_strategy(self, state: TradingState):
        ...
        intercept = 5012
        coeff = 5.016e-05
        lin_reg_price = coeff * state.timestamp + intercept

        orders = []
        order_depth: OrderDepth = state.order_depths[STARFRUIT]
        orders: List[Order] = []
        acceptable_bid_price = math.floor(lin_reg_price) + \
            self.price_trend[STARFRUIT]
        acceptable_ask_price = math.ceil(lin_reg_price) + \
            self.price_trend[STARFRUIT]
        print("Acceptable price : " + str(lin_reg_price))
    ...
    def run(self, state: TradingState):
        ...
        print(f"Log round {self.round}, timestamp {state.timestamp}")
